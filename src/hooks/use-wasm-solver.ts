...
export function useWasmSolver() {
  ...
  useEffect(() => {
    ...
    async function loadWasm() {
      try {
        ...
        if (isMounted) {
          setError(
            `Failed to load Sudoku solver: ${err instanceof Error ? err.message : String(err)}`,
          );
          setIsLoading(false);
        }
      }
  ...
  const solvePuzzle = useCallback(
    async (puzzleStr: string): Promise<number[][] | null> => {
      if (!wasmModule?.ccall) {
  ...
  const solveBatch = useCallback(
    async (
      ...
      if (
        !wasmModule?._solveBatch ||
        !wasmModule._allocateInputBuffer ||
        !wasmModule._allocateOutputBuffer ||
        !wasmModule._allocateSolvedFlags ||
        !wasmModule._setPuzzle ||
        !wasmModule._getSolution ||
        !wasmModule._wasSolved ||
